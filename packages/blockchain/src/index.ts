...
export class Blockchain implements BlockchainInterface {
  ...
  private async _init(genesisBlock?: Block): Promise<void> {
    let dbGenesisBlock;
    try {
      const genesisHash = await this.database.numberToHash(new BN(0));
      dbGenesisBlock = await this.database.getBlock(genesisHash);
    } catch (error: any) {
    ...
    try {
      const heads = await this.database.getHeads();
      this._heads = heads;
    } catch (error: any) {
      if (error.type !== 'NotFoundError') {
        throw error;
    ...
    try {
      const hash = await this.database.getHeadHeader();
  ...
  private async _getBlock(blockId: Buffer | number | BN) {
    return this.database.getBlock(blockId);
  }
  ...
  public async getTotalDifficulty(hash: Buffer, number?: BN): Promise<BN> {
    if (!number) {
      number = await this.database.hashToNumber(hash);
    }
    return this.database.getTotalDifficulty(hash, number);
  }
  ...
  async selectNeededHashes(hashes: Array<Buffer>): Promise<Buffer[]> {
    return await this.initAndLock<Buffer[]>(async () => {
      ...
      while (max >= min) {
        let number;
        try {
          number = await this.database.hashToNumber(hashes[mid]);
        } catch (error: any) {
          if (error.type !== 'NotFoundError') {
            throw error;
          }
  ...
  private async _iterator(
    ...
    return await this.initAndLock<number>(async (): Promise<number> => {
      ...
      const headBlockNumber = await this.database.hashToNumber(headHash);
  ...
  private async _saveHeads() {
    return this.database.batch(this._saveHeadOps());
  }
  ...
  private async _getHeader(hash: Buffer, number?: BN) {
    ...
    return this.database.getHeader(hash, number);
  }
  ...
  private async _getCanonicalHeader(number: BN) {
    const hash = await this.database.numberToHash(number);
  ...
  cliqueActiveSignersByBlockNumber(number: BN): Address[] {
    for (let i = this._cliqueLatestSignerStates.length - 1; i >= 0; i--) {
      const state = this._cliqueLatestSignerStates[i];
  ...
  cliqueCheckNextRecentlySigned(
    ...
    let signers = this._cliqueLatestBlockSigners;
    signers = signers.slice(signers.length < this.cliqueSignerLimit() ? 0 : 1);
