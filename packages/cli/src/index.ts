...
import {
  installStartAction,
  installAccountCommand,
  installAttachCommand,
...
program.option(
  '--rpc-api <apis>',
  'rpc server apis: debug, eth, net, txpool, web3, rei'
);
...
program.option(
  '--bootnodes <bootnodes...>',
  'comma separated list of bootnodes'
);
program.option('--sync <sync>', 'sync mode: full, snap', 'full');
program.option(
  '--snap-trusted-height <trustedHeight>',
...
program.option(
  '--chain <chain>',
  'chain name: rei-mainnet, rei-testnet, rei-devnet'
);
program.option('--mine', 'mine block');
program.option('--coinbase <address>', 'miner address');
program.option(
  '--verbosity <verbosity>',
  'logging verbosity: silent, error, warn, info, debug, detail',
  'info'
);
program.option(
  '--receipts-cache-size <receiptsCacheSize>',
  'receipts cache size'
);
program.option('--evm <evm>', 'evm implementation type, "js" or "binding"');
program.option('--bls <bls>', 'the datadir for bls', 'bls');
program.option(
  '--bls-password <blsPassword>',
  'bls password file to use for non-interactive password input'
);
