...
import { genesisStateByName } from './genesisStates';
...
export class Common extends EventEmitter {
  ...
  static isSupportedChainId(chainId: BN): boolean {
    return Boolean(chains['names'][chainId.toString()]);
  }

  static isSupportedChainName(name: string): boolean {
    return Boolean(name !== 'names' && chains[name]);
  }

  private static _getChainParams(chain: string | number | Chain | BN): IChain {
    if (typeof chain === 'number' || BN.isBN(chain)) {
      chain = chain.toString();

      if (chains['names'][chain]) {
        const name: string = chains['names'][chain];
        return chains[name];
      }
  ...
  constructor(opts: CommonOpts) {
    ...
    this.DEFAULT_HARDFORK =
      this._chainParams.defaultHardfork ?? Hardfork.Istanbul;
  ...
  setHardfork(hardfork: string | Hardfork): void {
    ...
    for (const hfChanges of HARDFORK_CHANGES) {
      if (
        this.hardforkIsActiveOnChain(hfChanges[0]) &&
        hfChanges[0] === hardfork
      ) {
  ...
  isActivatedEIP(eip: number): boolean {
    ...
    for (const hfChanges of HARDFORK_CHANGES) {
      if (this.hardforkIsActiveOnChain(hfChanges[0])) {
        ...
        if (
          this.gteHardfork(hf['name'], { onlyActive: true }) &&
          'eips' in hf
        ) {
   ...
   */
  activeHardforks(
    blockNumber?: BNLike | null,
    opts: hardforkOptions = {}
  ): Array<any> {
    const activeHardforks: any[] = [];
    const hfs = this.hardforks();
    for (const hf of hfs) {
  ...
  isNextHardforkBlock(
    ...
    return nextHardforkBlock === null
      ? false
      : nextHardforkBlock.eq(blockNumber);
  }
  ...
  _calcForkHash(hardfork: string | Hardfork) {
    ...
    const forkhash = intToBuffer(crc32Buffer(inputBuffer) >>> 0).toString(
      'hex'
    );
    return `0x${forkhash}`;
  }
  ...
  genesisState(): GenesisState {
    return genesisStateByName(this.chainName());
  }
  ...
  consensusType(): string | ConsensusType {
    ...
    const activeHardforkNames: string[] = this.activeHardforks().map(
      ({ name }) => name
    );
  ...
  consensusConfig(): any {
    ...
    for (const hfChanges of HARDFORK_CHANGES) {
      ...
      if ('consensus' in hfChanges[1]) {
        ...
        value =
          hfChanges[1]['consensus'][hfChanges[1]['consensus']['algorithm']];
      }
      if (hfChanges[0] === hardfork) break;
    }
    if (value) {
