...
describe('ValidatorBLS', () => {
  ...
  beforeEach(async () => {
    validatorBLS = await validatorBLSFactory
      .connect(deployer)
      .deploy(genesisAddress, genesisBLSPublickeys);
  });

  it('should get genesis validators BLS public key', async () => {
    for (let i = 0; i < genesisAddress.length; i++) {
      expect(await validatorBLS.getBLSPublicKey(genesisAddress[i])).to.equal(
        genesisBLSPublickeys[i]
      );
    }
  });
  ...
  it('should set validator BLS public key', async () => {
    ...
    expect(
      await validatorBLS.getBLSPublicKey(await user1.getAddress())
    ).to.equal(validatorBLSPublicKey);
    const validatorBLSPublicKey1 = getRandomBytes(48);
    await validatorBLS.connect(user1).setBLSPublicKey(validatorBLSPublicKey1);
    expect(
      await validatorBLS.getBLSPublicKey(await user1.getAddress())
    ).to.equal(validatorBLSPublicKey1);
  });
  ...
  it('should get the validator is registered BLS public key', async () => {
    expect(await validatorBLS.isRegistered(await user1.getAddress())).to.equal(
      false
    );
    await validatorBLS.connect(user1).setBLSPublicKey(getRandomBytes(48));
    expect(await validatorBLS.isRegistered(await user1.getAddress())).to.equal(
  ...
  it('should the BLS public key can only be set once', async () => {
    ...
    expect(await validatorBLS.isBLSPublicKeyExist(BLSPublicKey)).to.equal(
      false
    );
...
describe('ValidatorBLSFallback', () => {
  it('should set validator BLS public key', async () => {
    const fallBackFactory: ContractFactory = await ethers.getContractFactory(
      'ValidatorBLSFallback'
    );
    const fallback: Contract = await fallBackFactory.deploy();
    const validatorBLSFactory: ContractFactory =
      await ethers.getContractFactory('ValidatorBLS');
    const validatorBLS: Contract = await validatorBLSFactory.attach(
      fallback.address
    );
    try {
