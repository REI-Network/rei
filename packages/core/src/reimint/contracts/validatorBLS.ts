...
const event = {
  SetBLSPublicKey: toBuffer(
    '0x4861c1796b9ac9313a6c9d77539ee86af148464a0b9bfd9e7bcd50baae5ca9b2'
  )
};
...
export class ValidatorBLS extends Contract {
  constructor(evm: EVM, common: Common) {
    super(
      ...
      common,
      methods,
      Address.fromString(common.param('vm', 'postaddr'))
    );
  }
    ...
    for (const receipt of receipts) {
      if (receipt.logs.length > 0) {
        for (const log of receipt.logs) {
          if (
            log.address.equals(blsAddr.buf) &&
            log.topics.length === 2 &&
            log.topics[0].equals(event['SetBLSPublicKey'])
          ) {
            //get validator address and bls public key
            changes.setBlsPublicKey(
              bufferToAddress(log.topics[1]),
              log.data.slice(64, 112)
            );
          }
        }
      }
