 ...
 */
function nibblesCompare(n1: Nibbles, n2: Nibbles) {
 ...
 */
async function unset(trie: Trie, parent: TrieNode, child: TrieNode | null, key: Nibbles, pos: number, removeLeft: boolean, stack: TrieNode[]): Promise<number> {
  if (child instanceof BranchNode) {
     ...
     */
    if (key.length - pos < child._nibbles.length || nibblesCompare(child._nibbles, key.slice(pos, pos + child._nibbles.length)) !== 0) {
      if (removeLeft) {
    ...
    if (child instanceof LeafNode) {
      ...
      // continue to the next node
      return await unset(trie, child, _child, key, pos + child._nibbles.length, removeLeft, stack);
    }
  } else if (child === null) {
    return pos - 1;
  } else {
...
async function unsetInternal(trie: Trie, left: Nibbles, right: Nibbles): Promise<boolean> {
  ...
  while (true) {
    if (node instanceof ExtensionNode || node instanceof LeafNode) {
      ...
      if (left.length - pos < node._nibbles.length) {
        shortForkLeft = nibblesCompare(left.slice(pos), node._nibbles);
      } else {
        shortForkLeft = nibblesCompare(left.slice(pos, pos + node._nibbles.length), node._nibbles);
      }

      if (right.length - pos < node._nibbles.length) {
        shortForkRight = nibblesCompare(right.slice(pos), node._nibbles);
      } else {
        shortForkRight = nibblesCompare(right.slice(pos, pos + node._nibbles.length), node._nibbles);
      }
  ...
  if (node instanceof ExtensionNode || node instanceof LeafNode) {
    ...
    if (shortForkRight !== 0) {
      ...
      const endPos = await unset(trie, node, child, left.slice(pos), node._nibbles.length, false, stack);
    ...
    if (shortForkLeft !== 0) {
      ...
      const endPos = await unset(trie, node, child, right.slice(pos), node._nibbles.length, true, stack);
 ...
 */
async function verifyProof(rootHash: Buffer, key: Buffer, proof: Buffer[]): Promise<{ value: Buffer | null; trie: Trie }> {
...
async function hasRightElement(trie: Trie, key: Nibbles): Promise<boolean> {
  ...
  while (node !== null) {
    if (node instanceof BranchNode) {
      ...
      if (key.length - pos < node._nibbles.length || nibblesCompare(node._nibbles, key.slice(pos, pos + node._nibbles.length)) !== 0) {
