...
export class StakingAccount extends Account {
  ...
  static fromAccountData(accountData: StakingAccountData) {
    ...
    return new StakingAccount(
      ...
      stakeInfo ? StakeInfo.fromStakeInfoData(stakeInfo) : undefined
    );
  }
  ...
  public static fromValuesArray(values: Buffer[]) {
    ...
    if (values.length === 4) {
      ...
      return new StakingAccount(
        ...
        stateRoot,
        codeHash,
        stakeInfo ? StakeInfo.fromValuesArray(stakeInfo) : undefined
      );
    }
  }
  ...
  constructor(
    ...
    stateRoot = KECCAK256_RLP,
    codeHash = KECCAK256_NULL,
    stakeInfo: StakeInfo | undefined = undefined
  ) {
   ...
   */
  raw(): Buffer[] {
    return this.stakeInfo && !this.stakeInfo.isEmpty()
      ? [
          bnToUnpaddedBuffer(this.nonce),
          bnToUnpaddedBuffer(this.balance),
          this.stateRoot,
          this.codeHash,
          this.stakeInfo.raw() as unknown as Buffer
        ]
      : [
          bnToUnpaddedBuffer(this.nonce),
          bnToUnpaddedBuffer(this.balance),
  ...
  isEmpty(): boolean {
    return (
      ...
      (this.stakeInfo === undefined || this.stakeInfo.isEmpty())
    );
  }
  ...
  slimRaw(): Buffer[] {
    ...
    rawBuffer.push(
      this.stateRoot.equals(KECCAK256_RLP) ? Buffer.alloc(0) : this.stateRoot
    );
    rawBuffer.push(
      this.codeHash.equals(KECCAK256_NULL) ? Buffer.alloc(0) : this.codeHash
    );
