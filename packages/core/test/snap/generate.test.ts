...
describe('GenerateSnapshot', () => {
  ...
  before(async () => {
    const { root, accounts: _accounts } = await genRandomAccounts(
      db,
      200,
      10,
      false
  ...
  it('should generate succeed', async () => {
    ...
    expect(diskLayer.genMarker === undefined, 'should generate finished').be
      .true;
    ...
    for (const { address, account, storageData } of accounts) {
      ...
      expect(_account.equals(account.serialize()), 'account should be equal').be
        .true;
      ...
      for (const [storageHash, { key, val }] of storageData) {
        ...
        expect(_stoargeValue.equals(val), 'storage data should be equal').be
          .true;
     ...
     */
    expect(storage.toNumber()).be.equal(
      200 * (1 + 32 + 70) + 200 * 10 * (1 + 32 + 32 + 32)
    );
  ...
  it('should abort succeed', async () => {
    ...
    for (const { address, account, storageData } of accounts) {
      ...
      if (cmp < 0) {
        const _account = await db.getSerializedSnapAccount(accountHash);
        expect(_account.equals(account.serialize()), 'account should be equal')
          .be.true;
        for (const [storageHash, { val }] of storageData) {
          const _stoargeValue = await db.getSnapStorage(
            accountHash,
            storageHash
          );
          expect(_stoargeValue.equals(val), 'storage data should be equal').be
        ...
        for (const [storageHash] of storageData) {
          await shouldNotExist(() =>
            db.getSnapStorage(accountHash, storageHash)
          );
        }
     ...
     */
    const total =
      _accounts.toNumber() * (1 + 32 + 70) +
      slots.toNumber() * (1 + 32 + 32 + 32);
