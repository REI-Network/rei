...
describe('Layer', () => {
  describe('DiskLayer', () => {
    ...
    it('should get account and storage data succeed', async () => {
      for (const { address, account, storageData } of accounts) {
        ...
        expect(
          _account.serialize().equals(account.serialize()),
    ...
    it('should iterate account succeed', async () => {
      ...
      for await (const { hash, getValue } of diskLayer.genAccountIterator(
        EMPTY_HASH
      )) {
        ...
        expect(
          _accounts[index].account.serialize().equals(getValue().serialize()),
    ...
    it('should iterate storage data succeed', async () => {
      for (const { address, storageData: _storageData } of accounts) {
        ...
        const accountHash = keccak256(address);
        const { iter, destructed } = diskLayer.genStorageIterator(
          accountHash,
          EMPTY_HASH
        );
  ...
  describe('DiffLayer', () => {
    before(async () => {
      ...
      for (let i = 0; i < 3; i++) {
        if (i === 0) {
          ...
          layers.push({
            layer: accountsToDiffLayer(
              latest.layer as Snapshot,
              root,
              accounts
            ),
            accounts
    ...
    it('should get account succeed(when account exists in this diff layer)', async () => {
      ...
      const expectAccount =
        accounts[getRandomIntInclusive(0, accounts.length - 1)];
      const accountHash = keccak256(expectAccount.address);
      expect(
        (layer as DiffLayer).diffed.check(accountHash),
        'should hit diff layer bloom'
      ).be.true;
      const account = await layer.getAccount(accountHash);
      expect(
        account?.serialize()?.equals(expectAccount.account.serialize()),
        'should be equal'
      ).be.true;
    });
    ...
    it('should get storage data succeed(when storage data exists in this diff layer)', async () => {
      ...
      const storageHash =
        storageHashes[getRandomIntInclusive(0, storageHashes.length - 1)];
      const storageData = await layer.getStorage(accountHash, storageHash);
      expect(
        storageData?.equals(expectAccount.storageData.get(storageHash)!.val),
        'should be equal'
      ).be.true;
    });
    ...
    it('should get account succeed(when account does not exist in this diff layer but exist in parent diff layer)', async () => {
      ...
      const index = _accounts.findIndex(
        ({ address }) =>
          accounts.findIndex(({ address: _address }) =>
    ...
    it('should iterate account succeed', async () => {
      ...
      for (const { layer, accounts } of diffLayers) {
        const _accounts = [...accounts];
        for await (const { hash, getValue } of layer.genAccountIterator(
          EMPTY_HASH
        )) {
          const index = _accounts.findIndex(({ address }) =>
            keccak256(address).equals(hash)
    ...
    it('should iterate storage data succeed', async () => {
      ...
      for (const { layer, accounts } of diffLayers) {
        for (const { address, storageData: _storageData } of accounts) {
          ...
          const accountHash = keccak256(address);
          const { iter, destructed } = layer.genStorageIterator(
            accountHash,
            EMPTY_HASH
          );
          expect(destructed, 'should not be destructed').be.false;
  ...
  describe('FastIterator', () => {
    it('should fast iterate account succeed', async () => {
      ...
      let totalCount = 0;
      for await (const { hash, value } of new FastSnapIterator(
        layer,
        (snap) => {
          return {
        ...
        expect(
          expectAccount?.serialize().equals(value.serialize()),
          'account should be equal'
        ).be.true;
        totalCount++;
      }
      expect(totalCount, 'total count should be equal').be.equal(
        layers[0].accounts.length
      );
    });
    ...
    it('should fast iterate storage data succeed', async () => {
      ...
      for (const { address } of layers[0].accounts) {
        ...
        for await (const { hash, value } of new FastSnapIterator(
          layer,
          (snap) => {
            const { iter, destructed } = snap.genStorageIterator(
              accountHash,
              EMPTY_HASH
            );
            return {
    ...
    it('should skip empty buffer', async () => {
      ...
      for await (const { hash: _hash, value: _value } of new FastSnapIterator(
        layer,
        (snap) => {
          const { iter, destructed } = snap.genStorageIterator(
            accountHash,
