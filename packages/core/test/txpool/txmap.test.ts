...
describe('TxSortedMap', () => {
  ...
  it('should resize correctly', () => {
    ...
    expect(
      result[0].serialize().equals(testTransactions[2].serialize()),
      'transaction should be equal'
    ).be.true;
  });
  ...
  it('should filter correctly', () => {
    ...
    const { removed, invalids } = txSortedMap.filter(
      hexStringToBN('0x5207'),
      hexStringToBN('0x5207')
    );
  ...
  it('should ready correctly', () => {
    ...
    expect(
      readies[0].serialize().equals(testTransactions[0].serialize()),
      'transaction should be true'
    ).be.true;
  });
  ...
  it('should clear correctly', () => {
    ...
    results.forEach((trx, i) => {
      expect(
        trx.serialize().equals(testTransactions[i + 1].serialize()),
        'transaction should be equal'
      ).be.true;
    });
