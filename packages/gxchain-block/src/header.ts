import { Address, BN, bnToHex, KECCAK256_RLP_ARRAY, KECCAK256_RLP, rlp, rlphash, toBuffer, unpadBuffer, zeros } from 'ethereumjs-util';
...
export class BlockHeader {
   ...
   */
  constructor(parentHash: Buffer, uncleHash: Buffer, coinbase: Address, stateRoot: Buffer, transactionsTrie: Buffer, receiptTrie: Buffer, bloom: Buffer, difficulty: BN, number: BN, gasLimit: BN, gasUsed: BN, timestamp: BN, extraData: Buffer, mixHash: Buffer, nonce: Buffer, options: BlockOptions = {}) {
    if (options.common) {
  ...
  validateGasLimit(parentBlockHeader: BlockHeader): boolean {
    ...
    return gasLimit.lt(maxGasLimit) && gasLimit.gt(minGasLimit) && gasLimit.gte(this._common.paramByHardfork('gasConfig', 'minGasLimit', hardfork));
  }
