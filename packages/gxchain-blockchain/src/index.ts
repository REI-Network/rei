...
import { Database, DBOp, DBSetBlockOrHeader, DBSetTD, DBSetHashToNumber, DBSaveLookups, DBTarget } from '@gxchain2/database';
...
export class Blockchain extends EventEmitter {
  ...
  private async _iterator(name: string, onBlock: OnBlock, maxBlocks?: number) {
    await this.initAndLock<void>(async () => {
      ...
      const number = await this.database.hashToNumber(blockHash);
