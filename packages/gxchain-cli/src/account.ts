...
export function installAccountCommand(program: any) {
  ...
  account
    ...
    .action((options) => {
      ...
      const manager = new AccountManager(getKeyDataDir(options.keydatadir));
      const { address, path } = manager.newAccount(passphrase);
  ...
  account
    ...
    .action(async (options) => {
      ...
      try {
        ...
        manager.update(options.address, answer1.password, answer2.newpassword);
      } catch (err) {
  ...
  account
    ...
    .action(async (keydir, options) => {
      ...
      if (update) {
        ...
        console.log('Address : ', toChecksumAddress(manager.importKeyByPrivateKey(privateKey, answer1.password)));
      }
    });
}
