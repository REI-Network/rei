...
export function updateSstoreGasEIP2200(runState: RunState, found: any, value: Buffer, key: Buffer) {
  ...
  if (current.equals(value)) {
    ...
    return runState.eei.useGas(new BN(adjustSstoreGasEIP2929(runState, key, sstoreNoopCost, 'noop')));
  }
  ...
  if (original.equals(value)) {
    if (original.length === 0) {
      ...
      runState.eei.refundGas(new BN(adjustSstoreGasEIP2929(runState, key, sstoreInitRefund, 'initRefund')));
    } else {
      // Reset to original existing slot
      const sstoreCleanRefund = runState._common.param('gasPrices', 'sstoreCleanRefundEIP2200');
      runState.eei.refundGas(new BN(adjustSstoreGasEIP2929(runState, key, sstoreCleanRefund, 'cleanRefund')));
    }
  }
  // Dirty update
