...
export function accessAddressEIP2929(runState: RunState, address: Address, baseFee?: number) {
  ...
  if (!runState.accessedAddresses.has(addressStr)) {
    ...
    if (baseFee !== undefined) {
      runState.eei.useGas(new BN(runState._common.param('gasPrices', 'coldaccountaccess') - baseFee));
    }
 ...
 */
export function adjustSstoreGasEIP2929(runState: RunState, key: Buffer, defaultCost: number, costName: string): number {
