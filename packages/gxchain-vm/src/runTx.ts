...
async function _runTx(this: VM, opts: RunTxOpts): Promise<RunTxResult> {
  ...
  if (!opts.skipBalance) {
    ...
    if (balance.lt(cost)) {
      throw new Error(`sender doesn't have enough funds to send tx. The upfront cost is: ${cost.toString()} and the sender's account only has: ${balance.toString()}`);
    }
  } else if (!opts.skipNonce) {
    if (!nonce.eq(tx.nonce)) {
      throw new Error(`the tx doesn't have the correct nonce. account has nonce of: ${nonce.toString()} tx has nonce of: ${tx.nonce.toString()}`);
    }
  }
