...
export class Block {
  ...
  toRPCJSON(isPending: boolean, fullTransactions: boolean) {
    return {
      number: isPending ? null : bnToHex(this.header.number),
      hash: isPending ? null : bufferToHex(this.hash()),
      parentHash: bufferToHex(this.header.parentHash),
      nonce: isPending ? null : bufferToHex(this.header.nonce),
      sha3Uncles: bufferToHex(this.header.uncleHash),
      logsBloom: isPending ? null : bufferToHex(this.header.bloom),
      transactionsRoot: bufferToHex(this.header.transactionsTrie),
      stateRoot: bufferToHex(this.header.stateRoot),
      receiptsRoot: bufferToHex(this.header.receiptTrie),
      miner: this.header.coinbase.toString(),
      mixHash: bufferToHex(this.header.mixHash),
      difficulty: bnToHex(this.header.difficulty),
      totalDifficulty: bnToHex(this.header.number),
      extraData: bufferToHex(this.header.extraData),
      size: intToHex(this.size),
      gasLimit: bnToHex(this.header.gasLimit),
      gasUsed: bnToHex(this.header.gasUsed),
      timestamp: bnToHex(this.header.timestamp),
      transactions: fullTransactions
        ...
        : this.transactions.map((tx) => bufferToHex(tx.hash())),
      uncles: this.uncleHeaders.map((uh) => bufferToHex(uh.hash()))
    };
  }
}
