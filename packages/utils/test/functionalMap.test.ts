...
const testdata = [
  '0xc275b0192c987dc235303aed7535a1c268fa786421bbe51643ffc671fd3fc5d7',
  ...
  '0xa2d39d66f541c087e6736636b479d38f86dc4cab85bf35d9356495125b893ffe',
  '0x02abe0a061c42bd49192e2174be7de7de842a00c3a415f7b01c072d443d6b305',
  '0x39ad914753bdc0f29680d43960c6aa92ab2896cd11269cb35e3f89e27b87168b'
];
...
describe('FunctionalMap', () => {
  const bufferToStringMap = new FunctionalBufferMap<string>();

  it('should get map size', () => {
    testdata.forEach((blockhash, i) => {
      bufferToStringMap.set(testBuffer[i], blockhash);
    });
    expect(bufferToStringMap.size, 'map size should be equal').be.equal(
      testBuffer.length
    );
  });
  ...
  it('should has the hash', () => {
    ...
    expect(bufferToStringMap.has(key), 'function result should be true').be
      .true;
  });

  it('should get value', () => {
    const key = testBuffer[0];
    expect(bufferToStringMap.get(key), 'got value should be equal').be.equal(
      testdata[0]
    );
  });
  ...
  it('should delete element', () => {
    const key = testBuffer[0];
    expect(bufferToStringMap.delete(key), 'function result should be true').be
      .true;
    expect(bufferToStringMap.has(key), 'function result should be false').be
      .false;
    expect(bufferToStringMap.size, 'map size should be equal').be.equal(
      testBuffer.length - 1
    );
...
describe('FunctionalSet', () => {
  const bufferSet = new FunctionalBufferSet();

  it('should get set size', () => {
    testBuffer.map((blockhash) => {
      bufferSet.add(blockhash);
    });
    expect(bufferSet.size, 'set size should be equal').be.equal(
      testdata.length
    );
  });
  ...
  it('should has the hash', () => {
    ...
    expect(
      bufferSet.has(hexStringToBuffer(key)),
      'function result should be true'
    ).be.true;
  });
  ...
  it('should get values', () => {
    ...
    Array.from(values).forEach((value, i) => {
      expect(value.equals(testBufferSorted[i]), 'value should be euqal').be
        .true;
    });
  });
