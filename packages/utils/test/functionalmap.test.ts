...
describe('FunctionalMap', () => {
  ...
  before(() => {
    testdata.map((blockhash) => {
      bufferToStringMap.set(hexStringToBuffer(blockhash), blockhash);
      bufferArray.push(hexStringToBuffer(blockhash));
    });
  });

  it('should get map size', () => {
    expect(bufferToStringMap.size, 'map size should be equal').be.equal(bufferArray.length);
  });

  it('should has the hash', () => {
    const key = bufferArray[0];
  ...
  it('should get values', () => {
    ...
    Array.from(values).forEach((value, i) => {
      expect(value, 'value should be euqal').be.equal(testdata[i]);
    });
  });
  ...
  it('should get keys', () => {
    ...
    Array.from(keys).forEach((key, i) => {
      expect(key.equals(bufferArray[i]), 'key should be euqal').be.true;
    });
  });
  ...
  it('should delete element', () => {
    ...
    expect(bufferToStringMap.size, 'map size should be equal').be.equal(bufferArray.length - 1);
  });
...
describe('FunctionalSet', () => {
  ...
  before(() => {
    testdata.map((blockhash) => {
      bufferSet.add(hexStringToBuffer(blockhash));
    });
  });
  ...
  it('should get values', () => {
    ...
    Array.from(values).forEach((value, i) => {
      expect(value.equals(hexStringToBuffer(testdata[i])), 'value should be euqal').be.true;
    });
  });
  ...
  it('should get keys', () => {
    ...
    Array.from(keys).forEach((key, i) => {
      expect(key.equals(hexStringToBuffer(testdata[i])), 'key should be euqal').be.true;
    });
  });
  ...
  it('should delete element', () => {
    const key = testdata[0];
    expect(bufferSet.delete(hexStringToBuffer(key)), 'function result should be true').be.true;
