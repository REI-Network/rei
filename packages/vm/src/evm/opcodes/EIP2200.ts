...
export function updateSstoreGasEIP2200(
  ...
  if (
    runState.eei
      .getGasLeft()
      .lten(common.param('gasPrices', 'sstoreSentryGasEIP2200'))
  ) {
  ...
  if (currentStorage.equals(value)) {
    ...
    return runState.eei.useGas(
      new BN(
        adjustSstoreGasEIP2929(runState, key, sstoreNoopCost, 'noop', common)
      ),
  ...
  if (originalStorage.equals(value)) {
    if (originalStorage.length === 0) {
      ...
      const sstoreInitRefund = common.param(
        'gasPrices',
      ...
      const sstoreCleanRefund = common.param(
        'gasPrices',
