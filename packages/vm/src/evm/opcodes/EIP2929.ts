...
export function accessAddressEIP2929(
  ...
  // Cold
  if (
    !(<EIP2929StateManager>runState.stateManager).isWarmedAddress(addressStr)
  ) {
...
export function accessStorageEIP2929(
  ...
  const slotIsCold = !(<EIP2929StateManager>(
    runState.stateManager
  )).isWarmedStorage(address, key);
  ...
  if (slotIsCold) {
    ...
    runState.eei.useGas(
      new BN(common.param('gasPrices', 'coldsload')),
      'EIP-2929 -> coldsload'
    );
  } else if (!isSstore) {
    runState.eei.useGas(
...
export function adjustSstoreGasEIP2929(
  ...
  if (
    (<EIP2929StateManager>runState.stateManager).isWarmedStorage(address, key)
  ) {
    switch (costName) {
      case 'reset':
