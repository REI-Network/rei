 ...
 */
export function accessAddressEIP2929(runState: RunState, address: Address, common: Common, chargeGas = true, isSelfdestruct = false) {
  ...
  if (!(<EIP2929StateManager>runState.stateManager).isWarmedAddress(addressStr)) {
    ...
    if (chargeGas) {
      runState.eei.useGas(new BN(common.param('gasPrices', 'coldaccountaccess')), 'EIP-2929 -> coldaccountaccess');
    }
    // Warm: (selfdestruct beneficiary address reads are not charged when warm)
  } else if (chargeGas && !isSelfdestruct) {
    runState.eei.useGas(new BN(common.param('gasPrices', 'warmstorageread')), 'EIP-2929 -> warmstorageread');
  }
}
 ...
 * @param {Common} common
 */
export function accessStorageEIP2929(runState: RunState, key: Buffer, isSstore: boolean, common: Common) {
  if (!common.isActivatedEIP(2929)) return;
 ...
 */
export function adjustSstoreGasEIP2929(runState: RunState, key: Buffer, defaultCost: number, costName: string, common: Common): number {
