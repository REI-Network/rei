...
export default async function runTx(
  ...
  opts.block =
    opts.block ?? Block.fromBlockData({}, { common: opts.tx.common });
  ...
  if (
    ...
    this._common.isActivatedEIP(2718)
  ) {
    ...
    if (
      ...
      !this._common.isActivatedEIP(1559)
    ) {
      await state.revert();
...
async function _runTx(
  ...
  if (this.DEBUG) {
    debugGas(
      `Subtracting base fee (${basefee}) from gasLimit (-> ${gasLimit})`
    );
  }
  ...
  if (!opts.skipBalance) {
    ...
    if (tx.supports(Capability.EIP1559FeeMarket)) {
      ...
      const cost = tx.gasLimit
        .mul((tx as FeeMarketEIP1559Transaction).maxFeePerGas)
        .add(tx.value);
      if (balance.lt(cost)) {
        throw new Error(
  ...
  const maxRefundQuotient = this._common.param(
    'gasConfig',
    'maxRefundQuotient'
  );
  if (!gasRefund.isZero()) {
    ...
    if (this.DEBUG) {
      debug(
        `Subtract tx gasRefund (${gasRefund}) from gasUsed (-> ${results.gasUsed})`
      );
    }
  } else {
    if (this.DEBUG) {
  ...
  if (opts.assignTxReward) {
    ...
    if (this.DEBUG) {
      debug(
        `tx update miner account (${miner.toString()}) balance (-> ${
  ...
  const cumulativeGasUsed = (opts.blockGasUsed ?? block.header.gasUsed).add(
    results.gasUsed
  );
  results.receipt = await (opts.generateTxReceipt ?? generateTxReceipt).bind(
    this
  )(tx, results, cumulativeGasUsed);
  ...
  if (this.DEBUG) {
    debug(
      `Generate tx receipt transactionType=${
        tx.type
      } gasUsed=${cumulativeGasUsed.toString()} bitvector=${short(
        baseReceipt.bitvector
      )} (${baseReceipt.bitvector.length} bytes) logs=${
        baseReceipt.logs.length
      }`
    );
  }
